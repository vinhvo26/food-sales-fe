<script setup>
import { useServicesAdminStore } from './store.js'
...
const useNewsAdmin = useServicesAdminStore()
...
onMounted(() => {
  if (route.params.id) {
    handleGetDetailNews(route.params.id)
  }
})
...
const handleGetDetailNews = async id => {
  ...
  if (res.status) {
    ...
    router.push(PATH.ADMIN_SERVICE)
  }
}
...
const handleCreateNews = async () => {
  try {
    if (!title.value) {
      toast.add({
        title: 'Notification',
        description: 'Thiếu tiêu đề dịch vụ',
    ...
    if (!content.value) {
      toast.add({
        title: 'Notification',
        description: 'Thiếu nội dung dịch vụ',
        icon: 'i-heroicons-exclamation-circle-20-solid',
        color: 'red',
      })
      return false
...
<template>
  <div>
    <NuxtLayout name="admin">
      <Head>
        <Title>{{
          route.params.id ? 'Cập nhật bài đăng dịch vụ' : 'Tạo bài đăng dịch vụ'
        }}</Title>
      </Head>
      <div class="detail-main">
        <div class="flex gap-x-6">
          <h1 v-if="!route.params.id">Tạo bài đăng dịch vụ</h1>
          <h1 v-else>Cập nhật bài đăng dịch vụ</h1>
        </div>
        <div class="detail-body mt-3 w-full flex flex-col gap-y-3">
          ...
          <div>
            <label>Hình ảnh đại diện dịch vụ</label>
            <form>
              <label class="block">
      ...
      <ModalConfirm
        :message="
          statusChange === NewsStatus.ACTIVE
            ? 'Bạn có chắc duyệt bài dịch vụ này không?'
            : 'Bạn có chắc huỷ duyệt bài dịch vụ này không?'
        "
        :isOpen="openModal"
        @handleConfirm="
          () => {
