    ...
    <NuxtLayout name="admin">
      <div class="news-main">
        ...
        <div class="flex justify-end">
          <UButton
            :to="PATH.ADMIN_INTRODUCE_CREATE"
        ...
        <div class="flex mt-3 search-user flex-wrap gap-4">
          <div class="w-64">
            <label for="">Tìm kiếm giới thiệu</label>
            <UInput
              v-model="key"
              size="lg"
              variant="outline"
          ...
          <div class="w-72">
            ...
            <VueDatePicker
              ...
              v-model="date"
              auto-apply
              placeholder="Ngày tạo giới thiệu"
              range
        ...
        <div class="table-news">
          ...
          >
            ...
            <template #action-data="{ row }">
                ...
                <UButton
                  @click="
                    () => {
                      ...
                      idIntroductionChange = row.new_id
                    }
                  "
              ...
              >
                <UButton
                  :to="PATH.ADMIN_INTRODUCE + `/${row.slug}`"
      ...
      <ModalConfirm
        :message="
          statusChange === NewsStatus.ACTIVE
            ...
            : statusChange === NewsStatus.DELETE
            ? 'Bạn có chắc xoá bài giới thiệu này không?'
...
const idIntroductionChange = ref()
...
const rows = computed(() => {
  return useIntroduceAdmin.listIntroduction.map((item, index) => ({
    ...item,
    index: index + 1,
  }))
})
...
const handleListIntroduction = async () => {
  loading.value = true
  const data = {
    key: key.value,
    startDate:
  ...
  const res = await useIntroduceAdmin.getListIntroduction(page.value, data)
  if (res.status) {
    useIntroduceAdmin.listIntroduction = res.data.data || []
    totalPage.value = res.data.total
    pageCount.value = res.data.per_page
  } else {
    useIntroduceAdmin.listIntroduction = []
    toast.add({
      title: 'Notification',
...
const handleChangeStatus = async () => {
  try {
    ...
    if (statusChange.value === NewsStatus.DELETE) {
      const { data } = await useIntroduceAdmin.deleteIntroduction(
        idIntroductionChange.value
      )
      if (data.status) {
        await handleListIntroduction()
