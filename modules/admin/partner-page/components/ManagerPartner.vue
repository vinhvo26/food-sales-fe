<template>
  <div class="news-main">
    ...
    <div class="flex mt-3 search-user flex-wrap gap-4">
      <div class="w-64">
        <label for="">Tìm kiếm đối tác</label>
        <UInput
          v-model="key"
          size="lg"
          variant="outline"
    ...
    <div class="table-news">
      ...
      >
        ...
        <template #action-data="{ row }">
          ...
          >
            <UButton
              :to="PATH.ADMIN_PARTNER + `/${row.slug}`"
  ...
  <ModalConfirm
    :message="
      statusChange === NewsStatus.ACTIVE
        ...
        : statusChange === NewsStatus.DELETE
        ? 'Bạn có chắc xoá bài đối tác này không?'
...
const rows = computed(() => {
  return useNewsAdmin.listPartner.map((item, index) => ({
    ...item,
    index: index + 1,
  }))
})
...
const handleListNews = async () => {
  ...
  if (res.status) {
    useNewsAdmin.listPartner = res.data.data || []
    totalPage.value = res.data.total
    pageCount.value = res.data.per_page
  } else {
    useNewsAdmin.listPartner = []
    toast.add({
      title: 'Notification',
