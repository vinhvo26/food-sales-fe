<script setup>
import { usePartnerAdminStore } from './store.js'
...
const useNewsAdmin = usePartnerAdminStore()
...
const handleGetDetailNews = async id => {
  ...
  if (res.status) {
    ...
    router.push(PATH.ADMIN_PARTNER)
  }
}
...
const handleCreateNews = async () => {
  try {
    if (!title.value) {
      toast.add({
        title: 'Notification',
        description: 'Thiếu tiêu đề đối tác',
    ...
    if (!content.value) {
      toast.add({
        title: 'Notification',
        description: 'Thiếu nội dung đối tác',
        icon: 'i-heroicons-exclamation-circle-20-solid',
        color: 'red',
      })
      return false
...
<template>
  <div>
    <NuxtLayout name="admin">
      <Head>
        <Title>{{
          route.params.id ? 'Cập nhật bài đăng đối tác' : 'Tạo bài đăng đối tác'
        }}</Title>
      </Head>
      <div class="detail-main">
        <div class="flex gap-x-6">
          <h1 v-if="!route.params.id">Tạo bài đăng đối tác</h1>
          <h1 v-else>Cập nhật bài đăng đối tác</h1>
        </div>
        <div class="detail-body mt-3 w-full flex flex-col gap-y-3">
          ...
          <div>
            <label>Hình ảnh đại diện đối tác</label>
            <form>
              <label class="block">
      ...
      <ModalConfirm
        :message="
          statusChange === NewsStatus.ACTIVE
            ? 'Bạn có chắc duyệt bài đối tác này không?'
            : 'Bạn có chắc huỷ duyệt bài đối tác này không?'
        "
        :isOpen="openModal"
        @handleConfirm="
          () => {
