...
import MembersRegisterForConsultation from './components/MembersRegisterForConsultation.vue'
...
onMounted(() => {
  if (ROLE.SELLER === authStore.userInfoData.role) {
    activeTab.value = 2
  }
})
...
const tabs = [
  {
    tille: 'Quản lý thành viên',
    index: 1,
  },
  { tille: 'Thành viên đăng ký tư vấn', index: 2 },
]
</script>
  ...
  <div>
    <NuxtLayout name="admin">
      <div class="user-main">
        ...
        <component v-if="activeTab === 1" :is="ManageMember" />
        <component v-else :is="MembersRegisterForConsultation" />
      </div>
    </NuxtLayout>
  </div>
</template>
